...
def tts_worker():
    while True:
        try:
            ...
            try:
                audio = tts.speak(text, speaker=tts.speakers[0], speed=SPEED)
...
def play_audio_worker():
    while True:
        try:
            ...
            # pcm_data = librosa.effects.time_stretch(pcm_data, rate=0.9)
            # 播放音频（阻塞方式）
            sd.play(pcm_data, samplerate=44100, blocking=True)
            # 标记任务完成
            audio_queue.task_done()
        except queue.Empty:
            continue
