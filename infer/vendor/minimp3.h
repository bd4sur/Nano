...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    /* 【玲珑 patch】mp3dec_scratch_t 约 16KB，放栈上会撑爆本项目的渲染任务栈（12KB）。
       定义 MINIMP3_EXTERNAL_SCRATCH 后，scratch 改由调用方经 minimp3_set_scratch()
       注入外部分配的缓冲（本项目为进入音乐盒时按需分配的 PSRAM，退出时释放）。*/
    extern mp3dec_scratch_t *g_minimp3_scratch;
    if (!g_minimp3_scratch) { info->frame_bytes = 0; return 0; }
#define scratch (*g_minimp3_scratch)
#else
