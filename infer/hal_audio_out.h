...
// 运行时调节播放音量（0~255；供音乐盒等场景的音量调节，不影响 close 时恢复原音量）
void audio_out_set_volume(uint8_t volume);

// 系统级扬声器主音量（0~255）：
//   set —— 写入并缓存主音量、同时应用到扬声器硬件（由 ui_init/系统设置调用；
//          按键音、OFDM 发射初始音量、音乐盒初始音量均来源于它；
//          麦克风恢复音量由调用方以参数注入 mic_init，本层不跨 HAL 反向读取）；
//   get —— 读回当前主音量（供系统设置显示或调试读取）。
