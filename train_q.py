...
class TrainGPT():
    ...
    def get_batch(self, phase):
        dataset = self.train_data if phase == 'train' else self.val_data
        ix = torch.randint(int(len(dataset) / 11), (self.batch_size,))
        x = torch.stack([torch.from_numpy((dataset[i*11 : i*11+self.block_size]).astype(np.int64)) for i in ix])
        y = torch.stack([torch.from_numpy((dataset[i*11+self.block_size : i*11+self.block_size+1]).astype(np.int64)) for i in ix])
        x, y = x.to(self.device), y.to(self.device)
        return x, y
    ...
    def start(self):
        ...
        tb_writer = SummaryWriter(log_dir="log", comment='train_q')
        ...
        for iter in range(self.max_iters):
            ...
            if iter % self.log_interval == 0:
                ...
                tb_writer.add_scalar('loss@trainset', lossf, iter)
            self.iter_num += 1
