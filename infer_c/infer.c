...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < token_count; i++) {
        ...
        for(int j = 0; j < token->length; j++) {
            uint32_t id = token->ids[j];
            printf("%lc", GB_CHAR[id][0]);
        }
        printf(" | ");
    }
    printf("\n");
