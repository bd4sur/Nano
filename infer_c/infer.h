...
void read_checkpoint(char* checkpoint, ModelConfig* config, ModelParam* weights,
    ...
    fread(header, sizeof(uint32_t), header_uint_length, file);
    // if (fread(header, sizeof(uint32_t), header_uint_length, file) != 1) { exit(EXIT_FAILURE); }
    ...
    fread(&param_num, sizeof(unsigned long long), 1, file);
